 ...
 */
export async function setupPaymentMethod(token: string, successURL: string, cancelURL: string): Promise<string> {
    const url = billingBaseURL() + "/api/v1/billing/payment-method/setup";
    const response = await api.post(url, { success_url: successURL, cancel_url: cancelURL }, {
        headers: { "Authorization": "Bearer " + token },
    });
